...
const initializeDbAndServer = async () => {
  try {
    ...
    app.listen(3000, () =>
      console.log("Server Running at http://ocalhost:3000/")
    );
  } catch (e) {
...
app.get("/players/", async (request, response) => {
  ...
  response.send(
    playersArray.map((eachPlayer) => convertDbObjectToRespondObject(eachPlayer))
  );
});
...
app.get("/players/:playerId/", async (request, response) => {
  ...
  response.send(convertDbObjectToRespondObject(player));
});
